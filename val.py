...
def add_new_player(msg):
  name = msg.split('#')[0]
  tag = msg.split('#')[1]
  response = requests.get('https://api.henrikdev.xyz/valorant/v1/account/' + name + '/' + tag)
...
def update_player_data():
  ...
  for i in db["players"]:
    ...
    response = requests.get('https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/na/' + i + '?filter=unrated')
    if response.status_code == 200:
      json_data = json.loads(response.text)
      matches += json_data['data']
    
    response = requests.get('https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/na/' + i + '?filter=competitive')
    if response.status_code == 200:
      json_data = json.loads(response.text)
      matches += json_data['data']
