...
def add_new_player(msg):
    name = msg.split('#')[0]
    tag = msg.split('#')[1]
    response = requests.get('https://api.henrikdev.xyz/valorant/v1/account/' +
                            name + '/' + tag)
...
def update_player_data():
    ...
    for i in db["players"]:
        ...
        response = requests.get(
            'https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/na/' + i +
            '?filter=unrated')
        if response.status_code == 200:
            json_data = json.loads(response.text)
        ...
        for match in matches:
            ...
            for player in known_players:
                ...
                if matchid in db["players"][player['puuid']]['parsed_matches']:
                    ...
                    if (len(known_players) - 1) > db["players"][
                            player['puuid']]['parsed_matches'][matchid]:
                        db["players"][player['puuid']]['friend_score'] += len(
                            known_players) - 1 - db["players"][
                                player['puuid']]['parsed_matches'][matchid]
                    db["players"][player['puuid']]['parsed_matches'][
                        matchid] = len(known_players) - 1
                    continue
                else:
                    db["players"][player['puuid']]['parsed_matches'][
                        matchid] = len(known_players) - 1
                ...
                d['total_minutes'] += match['metadata'][
                    'game_length'] / 1000 / 60
